...
with open(scriptdir + '/veganmap-data.js', 'w') as f:
  ...
  for e in json['elements']:
    ...
    if typ == 'node':
      nodes[ide] = (lat,lon)
      if tags.get('diet:vegan') != 'yes' and tags.get('diet:vegan') != 'only': # nodes that are part of way (i.e. not accepting bitcoin)
        continue
